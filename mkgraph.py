...
##     print mygalaxy
     ...
     # Calculate the GMC's galactocentric distance.
...
##     print index
     ...
     # Plot the mass distribution trends.
     t = Table.read(galaxyname+'_data.fits')
     inneredge = t['Inner edge (kpc)'].data
     outeredge = t['Outer edge (kpc)'].data
     print outeredge, inneredge
     for i in range(n_bins):
          binmass = mass_sorted[index[i]:index[i+1]]
          ...
          # Format the plot.
          plt.legend(loc=0)
          plt.title(galaxyname+' Mass Distribution, bin '+repr(i))
          plt.xlabel(r'$M_\mathrm{\odot}$')
          plt.ylabel('CCDF')
          plt.text(min(binmass)*10,10**-2.5,'$R_g = $'+repr(inneredge[i])+' to '+repr(outeredge[i])+' kpc')
          plt.savefig(galaxyname+'_power_law_'+repr(i)+'.png')
          plt.close()
